...
function checkInput() {
    if (!this.value.match(/^(067|097|096|098|093|073|063|095|050)([0-9]{3})([0-9]{4})$/)) {
        ...
        alertBlock.lastElementChild.textContent = `Phone number entered is valid – ${this.value}`;
...
function requestCallback() {
    ...
    request.open(
        "POST",
        "https://csplatform.herokuapp.com/lektorium/initiateCall"
    );
    ...
    link.addEventListener("click", event => {
        ...
        if (event.target.classList.contains("active")) {
            ...
            if (event.target.dataset.parent === "sub") {
                subNav.setAttribute(
                    "style",
                    "display:flex;margin: 0 -40px;padding:5px 0;position: absolute;"
                );
            }
        }
    })
